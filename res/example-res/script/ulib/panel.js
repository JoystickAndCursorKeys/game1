...
class Button {
	...
	handle( buttonevent ) {
		...
		if( this.handlerobject != null && this.handlermethod != null ) {
			...
			try {
				this.handlerobject[ this.handlermethod ]( buttonevent.physicalButtonId );
			}
			catch(error) {
	...
	renderButton( )
	{
		...
		if( button.state == PPAINTR_BSTATE_NORMAL ) {
...
class ButtonPanel {
	...
	handlePanelDetailedMouseEvent( detailedEventType, posX, posY, buttonId ) {
		...
		if( activeButtonFound != null ) {
				...
				if( b.type == PPAINTR_BTYPE_VSLIDER ||  b.type == PPAINTR_BTYPE_HSLIDER) {
