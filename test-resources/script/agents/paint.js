class PaintAgent  {
	...
	activate() {
		...
		this.timer = setInterval(
			function() {
				...
				__this.view.timerUpdateView( paintCanvas, paintContext, overlay );
		...
		if( sig[ 0 ] == 'APPMOUSE' ) {
			...
			switch( sig[ 1 ] ) {
				case 'ENTER':
					...
					if( sig.data.buttonId == 'left' || sig.data.buttonId == 'right' ) {
						this.state.mouseState.updateButton( sig.data.buttonId , true )
					}
			...
			switch( sig[ 1 ] ) {
				...
				case 'SETFUNCTION':
					...
					break;
				case 'IMPORTIMAGE':
					var importstatus = this.state.importImage( sig.data.img, sig.data.name );

					if( !importstatus.resize ) {
				...
				case "SETSHAPEFILL":
					var options = sig.data;
					this.state.setShapeFillFlag( options.fill );
	...
	updateView() {
		this.view.updateView( this.state.paintCanvas, this.state.paintContext, this.state.overlay, this.brush );
	...
	resetView() {
		this.view.resetView( this.state.paintCanvas, this.state.paintContext, this.state.overlay  );
